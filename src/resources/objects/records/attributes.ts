...
export class Attributes extends APIResource {
  /**
   * Gets all values for a given attribute on a record. Historic values can be
   * queried using the `show_historic` query param. Historic values cannot be queried
   * on COMINT (Communication Intelligence) or enriched attributes and the endpoint
   * will return a 400 error if this is attempted. Historic values are sorted from
   * oldest to newest (by `active_from`). Some attributes are subject to billing
   * status and will return an empty array of values if theworkspace being queried
   ...
   * ```ts
   * const response =
   *   await client.objects.records.attributes.listValues(
  ...
  listValues(
    ...
    const { object, record_id, ...query } = params;
    return this._client.get(path`/v2/objects/${object}/records/${record_id}/attributes/${attribute}/values`, {
      query,
      ...options,
    });
  }
...
export interface AttributeListValuesParams {
  /**
   * Path param: A UUID or slug to identify the object the record belongs to.
   */
  object: string;

  /**
   * Path param: A UUID to identify the record you want to query values on.
   */
  record_id: string;
   ...
   * value(s). Defaults to `false`. Cannot be set to `true` for COMINT attributes or
   * enriched attributes on people/company objects.
   */
